...
- Location: tests\qaplayground_practice\dropdowns\dropdown.spec.ts:51:9
...
Call log:
  - waiting for getByRole('option', { name: 'TypeScript' })
...
```ts
  ...
  49 |     })
  50 | 
  51 |     test('TC04: Select Last Language and Read All Options', async({page}) =>{
  52 |         await page.getByTestId('language-select').click()
> 53 |         await page.getByRole('option', {name: 'TypeScript'}).click()
     |                                                              ^ Error: locator.click: Test timeout of 30000ms exceeded.
  54 | 
  55 |         const allvalues = page.locator('#languageSelect').allInnerTexts()
  56 |         await expect(page.locator('#languageSelect')).toHaveValue('Selected TypeScript; options: Python, Java, JavaScript, TypeScript')
  57 | 
  58 |     })
  59 | })
```